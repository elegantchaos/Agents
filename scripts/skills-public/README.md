...
These scripts support the shared skill workflow.
...
- `link-skill-repos.sh`: point `~/.agents/skills` at discovered skill directories
- `status-skill-repos.sh`: report uninitialized submodules, dirty repos, branch drift, repo-local skills, and symlink mismatches

These scripts are used by the `refresh` skill's global pass.

## Discovery

The scripts treat `skills/` as the source of truth.
They search each registered submodule, plus the repo-local operational skill allowlist, for the first `SKILL.md`, then use the front matter `name:` field as the runtime skill name.
